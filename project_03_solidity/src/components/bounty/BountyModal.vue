<template>
  <modal @closed="this.$emit('closed')">
    <template v-slot:header v-bind="$attrs">
      <card
      ...
      >
        <div class="p1">
          ...
          <div>Creator: {{ bounty.creator }}</div>
        </div>
      </card>
    </template>

    <template v-slot:subtitle v-bind="$attrs">
      <div
        v-bind="$attrs"
        v-if="bounty.assignee === '0x0000000000000000000000000000000000000000'"
      >
        <collective-button @collectiveClick="assignBounty">
          ASSIGN YOURSELF TO THIS BOUNTY & BECOME A GLORIOUS BUG-SLAYER
        </collective-button>
      </div>

      <div class="centerText" v-bind="$attrs" v-else>
        Bounty assigned to
        {{
    ...
    <template v-slot:body>
      <div
        ...
        v-if="bounty.assignee?.toLowerCase() === address.toLowerCase()"
      >
        You are assigned to this bounty, use the buttons below to signal that
        you have completed your job. The project owner will have to approve it
        afterwards.
        <collective-button
      ...
      <div
        ...
        "
      >
        As the project owner confirm that the bounty has been completed and
        release the funds to pay the bounty hunter.
        <collective-button v-if="bounty.isOpen" @collectiveClick="closeBounty">
          CONFIRM BOUNTY IS COMPLETED
        </collective-button>
        <div class="violet" v-else>
...
export default defineComponent({
  ...
  async mounted() {
    this.bounty = await this.contract.methods
      .fetchBounty(this.projectId, this.bountyId)
      .call()
  },
  methods: {
    async assignBounty() {
      await this.contract.methods
        .assignMeToBounty(this.projectId, this.bountyId)
        .send()
      this.bounty = await this.contract.methods
        .fetchBounty(this.projectId, this.bountyId)
        .call()
    ...
    async unassignBounty() {
      await this.contract.methods
        .unAssignMeFromBounty(this.projectId, this.bountyId)
        .send()
      this.bounty = await this.contract.methods
        .fetchBounty(this.projectId, this.bountyId)
        .call()
    ...
    async closeBounty() {
      await this.contract.methods
        .closeBounty(this.projectId, this.bountyId)
        .send()
      this.bounty = await this.contract.methods
        .fetchBounty(this.projectId, this.bountyId)
        .call()
